...
export const FeatureOrgDetailsNew = () => {
  const query = useQuery();
  const parent_dso_id = query.get("parent_dso_id");
  const dso_type = query.get("type") || "";
  const [data, setData] = useState<OrgData | null>(null);
  const fetchData = useCallback(async () => {
    try {
      const response: { data: { data: OrgData } } = await axios.get(
        `${
          import.meta.env.VITE_BASE_HOST
        }/dso/org-chart/get-parent-company-details?parent_dso_id=${parent_dso_id}`
      );
  ...
  return (
    <div className={styles.container}>
      ...
      <div>
        <Insights data={data} parent_dso_id={parent_dso_id || ""} dso_type={dso_type}/>
      </div>
    </div>
  );
};

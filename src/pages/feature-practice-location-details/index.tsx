...
export const FeaturePracticeLocationDetails = () => {
  const query = useQuery();
  const practice_location_id = query.get("practice_location_id");
  const [data, setData] = useState<PracticeLocationData | null>(null);
  const fetchData = useCallback(async () => {
    try {
      const response: { data: { data: PracticeLocationData } } = await axios.get(
        `${
          import.meta.env.VITE_BASE_HOST
        }/dso/org-chart/get-practice-company-details?practice_location_id=${practice_location_id}`
      );
  ...
  return (
    <div className={styles.container}>
      ...
      <div>
        <Insights data={data} practice_location_id={practice_location_id}/>
      </div>
    </div>
  );
};
